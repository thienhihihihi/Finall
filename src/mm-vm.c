...
if (vmaid==0) {
  pg_getval(caller->mm, currg->rg_start + offset, data, caller);
    ...
    pg_getval(caller->mm, currg->rg_end + offset, data, caller);
    ...
    uint32_t destination)
{
  ...
  if (proc->mm->symrgtbl[source].vmaid==0) {
   
  MEMPHY_dump1(proc->mram, proc->mm->symrgtbl[source].rg_start + offset, proc);}
  else{
    
    MEMPHY_dump1(proc->mram, proc->mm->symrgtbl[source].rg_end + offset, proc);
  }
#endif /* IODUMP */
  if (val == 0)
...
if (vmaid=0) {
  pg_setval(caller->mm, currg->rg_start + offset, value, caller);
    ...
    pg_setval(caller->mm, currg->rg_end + offset, value, caller);
    ...
    uint32_t offset)
{
  ...
  print_pgtbl(proc, 0, -1); /* print max TBL */
#endif /* PAGETBL_DUMP */
if (proc->mm->symrgtbl[destination].vmaid==0){
  MEMPHY_dump1(proc->mram, proc->mm->symrgtbl[destination].rg_start + offset, proc);
}else{
    MEMPHY_dump1(proc->mram, proc->mm->symrgtbl[destination].rg_end + offset, proc);
  ...
  if (vmaid == 1)
  {
    if (area->rg_end < cur_vma->vm_end) {// Tang kich thuoc vma neu viec cap phat reg hop le
      ...
      if (vm_map_ram(caller, area->rg_end, area->rg_start,
                 old_end-incnumpage*256, incnumpage, newrg) < 0)
  {
    free(area);
